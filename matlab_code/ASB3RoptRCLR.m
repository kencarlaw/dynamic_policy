...
aa=1;
bb=0.25; %0.1765;
mu=0.3;     %mean value of gi, individual benefit from ASA
sig=0.35;    %varance of gi 
%lam=13;     %socail cost conversion of individual ASA for social damage function
rho=2;
SWrho=5*rho;
mm=0;
...
opts.Sheet = "Sheet1";
opts.DataRange = "A2:D9";
opts.VariableNames = ["BE", "R1", "R2", "lam"];
opts.VariableTypes = ["double", "double", "double", "double"];
tbl = readtable("C:\Users\kcarlaw\Documents\MATLAB\ASB2022final\RCLR2.xlsx", opts, "UseExcel", false);
BE = tbl.BE;
R1 = tbl.R1;
R2 = tbl.R2;
lam = tbl.lam;
        ...
        %for r1=1:dim %1:r2
            ...
            while ((cc<1) && (b<MM))
                ...
                for t=1:Block
                    ...
                    NCw(t)=rho*R(t)+(lam(k)-1)*gbar3(t)+SWrho*swi(t);
                    NCw2(t)=rho*R(t)+(lam(k)-1)*gbar3(t);
                end
                if b<2
                    v=vw;
                    a=aw;
