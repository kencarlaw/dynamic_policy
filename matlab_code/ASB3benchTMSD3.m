...
% Updated Aug 01, 2023, K. I. Carlaw 
...
NN=N+1;     % enumeration of possible violations (0,...100)
F=1;        %Sanction if apprehended


%apprehension probability APR(v)=gam*(1-a^(-(R/v)))
gam=0.8;    
aa=1;       %Bayesian prior alpha
bb=0.25;    %Bayesian prior beta
...
% Calculating collapsed (on v) transition probability matrix (TP) and SD(R) for Markov process
vTP=zeros(NN,1);
aTP=zeros(NN,1);
SD=zeros(NN,NN);
SD2=zeros(NN,NN);
