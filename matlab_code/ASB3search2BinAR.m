    ...
    while crit>0.02 %RbbM>=crtit2
        ...
        for i=BEd:BEu
            ...
            while ((cc<1) && (b<MM))
                ...
                vz=zeros(Block,1);
                az=zeros(Block,1);
                q=zeros(Block,1);qb=zeros(Block,1);
                R=zeros(Block,1);
                gbar=zeros(Block,N);
                gbar3=zeros(Block,1);
                A=zeros(Block,1);
                    ...
                    if (b<2)
                        ...
                        if t<1+Z
                            if t<2
                                ...
                                vz(t)=sum(v(t+(b-1)*Block-Z:(b-1)*Block))+sum(vw(t-(Z-(Z-(t-1))):t-1));
                                az(t)=sum(a(t+(b-1)*Block-Z:(b-1)*Block))+sum(aw(t-(Z-(Z-(t-1))):t-1));  
                            end
                        else
                            vz(t)=sum(vw(t-Z:t-1));
                            az(t)=sum(aw(t-Z:t-1));
